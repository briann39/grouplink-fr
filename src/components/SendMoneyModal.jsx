...
const SendMoneyModal = ({ isOpen, onClose, onSuccess, userBalance, initialCbu, recipientName }) => {
  ...
  // Actualizar CBU cuando se pasa como prop
  useEffect(() => {
    if (initialCbu) {
      setCbu(initialCbu);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Verificar CBU y obtener datos del usuario destinatario
      const verifyResponse = await userService.verifyCBU(cbu.trim());
      
      if (verifyResponse.success) {
        setRecipientData(verifyResponse.user);
        setVerificationData({
          cbu: cbu.trim(),
          amount: amountNum,
          description: description.trim() || `Transferencia de ${amountNum} USD`,
        });
        setStep(2);
      }
    } catch (err) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Error al enviar dinero');
      setStep(1);
    } finally {
      setLoading(false);
    }
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          ...
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-gray-900">Enviar Dinero</h2>
            <button
              onClick={handleClose}
              ...
              {recipientName && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                  <p className="text-sm text-blue-800">
                    <strong>Enviando a:</strong> {recipientName}
                  </p>
                </div>
              )}
              <div>
          ...
          {/* Paso 2: Verificación */}
          {step === 2 && verificationData && recipientData && (
            <div className="space-y-4">
              {/* Información del usuario destinatario */}
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-sm text-blue-800 font-semibold mb-3">✅ Destinatario verificado:</p>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-gray-600">Nombre:</span>
                    <span className="font-semibold">{recipientData.fullName}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Email:</span>
                    <span className="font-semibold">{recipientData.email}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">CBU:</span>
                    <span className="font-semibold font-mono">{recipientData.cbu}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                <p className="text-sm text-gray-800 font-semibold mb-3">Detalles de la transacción:</p>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between border-t border-gray-300 pt-2 mt-2">
                    <span className="text-gray-700 font-medium">Total a descontar:</span>
                  ...
                  <div className="flex justify-between pt-2">
                    ...
                    <span className="font-semibold text-gray-900">
                      ${(parseFloat(userBalance || 0) - verificationData.amount - 1.00).toFixed(2)} USD
                    </span>
                  </div>
                  {verificationData.description && (
